function [Irp,Iheavy] = Depth_Doserp(E0,Rcsda,tau_ini,x)
format long
% tau_ini=0;
tau_straggle=0.0127*Rcsda^0.9352;
tau=sqrt(tau_ini^2+tau_straggle^2);
switch E0
    case E0<7
        tau_heavy=0.2e-6;
    case E0>7&&E0<=20.12
        tau_heavy=0.2e-6+1.108222*(E0-7)/(20.12-7);
    otherwise
...
%% calculate Ii
% x=0:0.1:40;
I1=(C1*tau_straggle-C4*(tau/Rcsda)^2*(Rcsda+x)).*1./(sqrt(2*pi)*tau).*exp(-(Rcsda-x).^2/(2*tau^2));
I2=(C2+C4/sqrt(pi)*(tau/Rcsda)^2)*1/2*(1+erf((Rcsda-x)/(sqrt(2)*tau)));
I3=C3*exp(2*((k1*tau_ini)/zmax)^2-2*k1*(Rcsda-x)/zmax)*1/2.*(1+erf((Rcsda-x)/(sqrt(2)*tau)-sqrt(2)*k1*tau/zmax));
I4=C4*(x/Rcsda).^2*1/2.*(1+erf((Rcsda-x)/(sqrt(2)*tau)));
% plot(x,I1,'-')
% hold on;
% plot(x,I2,'-')
% plot(x,I3,'-')
...
Nabs=1;
Irp=(0.042*xi.*x/Rcsda).*(I1+I2+I3+I4+Ilan1+Ilan2);
%% calculate Iheavy
if E0<=7
    Cheavy=0;
else
    Cheavy=0.00692+0.000156286*E0;
end
Iheavy=Cheavy*exp(-sqrt(pi)*x/zmax)*1/2.*(1+erf((Rcsda-x+E0/250-1)/(sqrt(2)*tau_heavy)));
% plot(x,Irp,'-')
% hold on
% plot(x,Iheavy,'-')
% Irp=Irp+Iheavy;
% legend('I1','I2','I3','I4','Ilan1','Ilan2','total')
end
